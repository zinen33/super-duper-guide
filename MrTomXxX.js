...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")(" () ❯ ") + chalk.hex("#00ffff")("ZINO )"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ ZINO ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
cron.schedule('0 0 7 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon humans, let's eat (Cradit:- ZINO)", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
